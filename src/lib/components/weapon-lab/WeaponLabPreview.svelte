<script lang="ts">
  ...
  const distanceDamage = $derived(
    preview.damageProfile.map((band) =>
      Math.max(1, Math.round(preview.damage * band))
    )
  );
</script>
...
<aside class="preview">
  <section class="preview-card">
    ...
    <div class="preview-details">
      <div class="stat-grid">
        <div><span>Shot damage</span><strong>{preview.damage}</strong></div>
        <div>
          <span>Melee damage</span><strong>{preview.meleeDamage}</strong>
        </div>
        <div>
          <span>Burst damage</span><strong>{preview.burstDamage}</strong>
        </div>
        <div>
      ...
      <div class="damage-bands">
        {#each preview.damageProfile as band, index}
          <div>
            <div class="damage-band-head">
              <span>{getDistanceBandLabel(index)}</span>
              <strong>{distanceDamage[index]}</strong>
            </div>
            <div class="bar">
              <div
                class="fill"
