<script lang="ts">
  ...
  interface MainMenuProps {
    ...
    onOpenSettings: () => void;
    onPlay: () => void;
    onUseCustomSeedChange: (enabled: boolean) => void;
    seed: string;
      ...
      <div class="new-game-row">
        ...
        <label class="seed-toggle">
          <input
            ...
            onchange={(event) =>
              onUseCustomSeedChange(event.currentTarget.checked)}
          >
      ...
      {#if useCustomSeed}
        <label class="seed-field">
          ...
          <input
            ...
            spellcheck="false"
            autocomplete="off"
            oninput={(event) => onCustomSeedChange(event.currentTarget.value)}
          >
        </label>
      {/if}
...
<style>
  ...
  .seed-field {
    ...
    gap: 0.28rem;
    justify-items: start;
    color: rgba(221, 205, 171, 0.62);
  }

  .seed-field input {
    box-sizing: border-box;
    inline-size: min(17rem, calc(100vw - 3rem));
    padding: 0.52rem 0.65rem;
    font:
      800 0.78rem "IBM Plex Mono",
      monospace;
    color: #fff3d1;
    outline: 0;
    background: rgba(8, 8, 10, 0.72);
    border: 1px solid rgba(221, 179, 106, 0.28);
    border-radius: 0.5rem;
    box-shadow: inset 0 1px 0 rgba(255, 241, 204, 0.05);
  }

  .seed-field input:focus {
    border-color: rgba(255, 242, 202, 0.64);
    box-shadow:
      inset 0 1px 0 rgba(255, 241, 204, 0.06),
      0 0 0.8rem rgba(221, 179, 106, 0.16);
  }
