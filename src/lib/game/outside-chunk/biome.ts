...
import { biomeIndex, type BiomeId, type ChunkSize } from "./types";
...
export const buildBiome = (p: BiomeParams): Uint8Array => {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const i = r * cols + c;
      let best = dist[i];
      if (c > 0) best = Math.min(best, dist[i - 1] + d1);
      if (r > 0) {
        best = Math.min(best, dist[i - cols] + d1);
        if (c > 0) best = Math.min(best, dist[i - cols - 1] + d2);
        if (c < cols - 1) best = Math.min(best, dist[i - cols + 1] + d2);
      }
      dist[i] = best;
    }
  }
  ...
  for (let r = rows - 1; r >= 0; r--) {
    for (let c = cols - 1; c >= 0; c--) {
      const i = r * cols + c;
      let best = dist[i];
      if (c < cols - 1) best = Math.min(best, dist[i + 1] + d1);
      if (r < rows - 1) {
        best = Math.min(best, dist[i + cols] + d1);
        if (c > 0) best = Math.min(best, dist[i + cols - 1] + d2);
        if (c < cols - 1) best = Math.min(best, dist[i + cols + 1] + d2);
...
export const biomeAtCell = (
  ...
  return (["water", "riverbank", "grassland", "forest", "scree", "cliff", "snow", "road"] as BiomeId[])[
    biome[r * cols + c]
  ];
};
